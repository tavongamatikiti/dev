...

	{
		...
		config = function()
			...
			for _, lang in ipairs({ "c", "cpp", "zig" }) do
				...
				table.insert(dap.configurations[lang], {
					name = "Launch current file (cc/zig run)",
					type = "codelldb",
					request = "launch",
					program = "${file}",
					cwd = "${workspaceFolder}",
			...
			-- Go 1.27 — delve (dlv) from ~/go/bin, gopls uses gofmt default
			local dlv_path = vim.fn.exepath("dlv") ~= "" and vim.fn.exepath("dlv") or vim.fn.expand("~/go/bin/dlv")
