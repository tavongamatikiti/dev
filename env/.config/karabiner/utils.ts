...
/**
 * Switch to an AeroSpace workspace and also bring the app to the front.
 * The `open -a` causes macOS to teleport to the app's native Space if it's
 * in macOS native fullscreen — which is desirable for cross-Space navigation.
 * Use for `Hyper+E+<app-letter>` bindings where the workspace is app-bound.
 */
export function switchToAppWorkspace(
	name: string,
	workspace: string,
): LayerCommand {
	...
	return {
		to: [
			{
				shell_command: `${aerospace} workspace '${workspace}'; open -a '${name}.app'`,
			},
		],
		description: `Switch to workspace ${workspace} (${name})`,
	};
}

/**
 * Open an app and keep it on a specific AeroSpace workspace.
 * If the app already has windows, sweep them to the target workspace and focus
 * one of them — never call `open -a`, because macOS will teleport to whichever
 * native Space the app lives on, breaking the AeroSpace workspace switch.
 * Only call `open -a` when there are no existing windows.
 */
export function appInWorkspace(
	name: string,
	bundleId: string,
	workspace: string,
): LayerCommand {
	...
	const script = [
		`wids=$(${aerospace} list-windows --monitor all --app-bundle-id '${bundleId}' --format '%{window-id}' 2>/dev/null)`,
		`if [ -n "$wids" ]`,
		`then echo "$wids" | while read -r w; do [ -n "$w" ] && ${aerospace} move-node-to-workspace --window-id "$w" '${workspace}'; done`,
		`${aerospace} workspace '${workspace}'`,
		`first=$(echo "$wids" | head -n1)`,
		`[ -n "$first" ] && ${aerospace} focus --window-id "$first"`,
		`else ${aerospace} workspace '${workspace}'`,
		`open -a '${name}.app'`,
		`fi`,
	...
	return {
		...
		description: `Open ${name} in workspace ${workspace}`,
	};
}
