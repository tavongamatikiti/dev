...
mkdir -p "$TEST_HOME/.tmux-sessions"
printf 'tmux-persist session\n' > "$TEST_HOME/.tmux-sessions/dawa"
...
cmp -s "$TEST_HOME/.tmux-sessions/dawa" "$backup_dir/.tmux-sessions/dawa" || fail 'tmux-persist session data was not backed up'
